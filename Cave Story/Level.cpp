...
void Level::loadMap(std::string mapName, Graphics &graphics) {
	...
	if (pTileset != NULL) {
		while (pTileset) {
			...
			//printf("%u",(unsigned)strlen(source));
...
{
	//center spawn point on spawn point object
	Vector2 _spawnPointCorrected = Vector2(_spawnPoint.x-(16 / 2 * globals::SPRITE_SCALE), _spawnPoint.y - (16 / 2 * globals::SPRITE_SCALE));
	
	return _spawnPointCorrected;
}
